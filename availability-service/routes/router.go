...
func RegisterRoutes(serviceClient *aztables.ServiceClient, rabbitMQService *service.RabbitMQService, publicKeyPEM string) *mux.Router {
	// Set up queues
	...
	r.Handle("/availability", middlewares.JWTMiddleware(publicKeyPEM, http.HandlerFunc(availabilityHandler.Create))).Methods(http.MethodPost)

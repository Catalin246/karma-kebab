...
def test_create_entity(mock_get_connection):
    ...
    db.create_entity = MagicMock(return_value={"plate_number": "123-ABC", "name": "King Karma", "status": "available", "description": "Can carry 120 kg kebab meat", "note": "all good"})
    create = db.create_entity(mock_get_connection)

    assert create["plate_number"] == "123-ABC"
    assert create["name"] == "King Karma"
    assert create["status"] == "available"
...
def test_list_all_entities(mock_get_connection):
    ...
    mock_cursor.fetchall.return_value = [
        {"plate_number": "123-ABC", "name": "King Karma", "status": "available", "description": "Can carry 120 kg kebab meat", "note": "all good"},
        {"plate_number": "456-DEF", "name": "Queen Karma", "status": "available", "description": "Can carry 220 kg kebab meat", "note": "fix front right lights"}
    ]
...
def test_create_truck_invalid_status(mock_table_operations):
    req = func.HttpRequest(
        method='POST',
        body=json.dumps({
            "plate_number": "123-ABC",
    ...
    assert "Invalid status" in resp.get_body().decode()
...
def test_return_truck_success(mock_table_operations):
    ...
    mock_return.return_value = {"plate_number": "123-ABC", "name": "King Karma", "status": "available", "description": "Can carry 120 kg kebab meat", "note": "all good"}
